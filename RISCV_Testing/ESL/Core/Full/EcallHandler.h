...
void EcallHandler::run() {
    while (true) {
        ...
        if (section == Sections::process) {
            ...
            reg_ecall_Port->read(regfileEcall);
            if (regfileEcall.reg_file_17 == SYS_exit) {
                // return to isa
                sysRES = false;
                ecall_isa_Port->write(sysRES);

                nextsection = Sections::idle;
            } else if (regfileEcall.reg_file_17 == SYS_write) {
                ans = regfileEcall.reg_file_12;
                index = 0;
        ...
        if (section == Sections::writing) {
            if ( index < regfileEcall.reg_file_12) {
                // Set up memory access
                memoryAccess.req = ME_RD;
                memoryAccess.mask = MT_BU; // reading a char
                memoryAccess.addrIn = regfileEcall.reg_file_11 + index; // a1 + index
